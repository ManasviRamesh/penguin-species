...
@st.cache()
def prediction(model , bill_length_mm,  bill_depth_mm , flipper_length_mm , body_mass_g):
  species = model.predict([[bill_length_mm, bill_depth_mm, flipper_length_mm, body_mass_g]])
  species = species[0]
  if species == 0:
    return "Adelie"
  elif species == 1:
...
sex = st.sidebar.checkbox('sex',('Male' , 'Female'))
island = st.sidebar.checkbox('island',('Biscoe' , 'Dream' , 'Torgersen'))

if st.sidebar.button("Predict"):
  if classifier == 'Support Vector Machine':
    species_type = prediction(svc_model, bill_len, bill_dep, flip_len, body_mass)
    score = svc_model.score(X_train, y_train)

  elif classifier =='Logistic Regression':
    species_type = prediction(log_reg , bill_len, bill_dep, flip_len, body_mass)
    score = log_reg.score(X_train, y_train)

  else:
    species_type = prediction(rf_clf, bill_len, bill_dep, flip_len, body_mass)
